...
import classes from './EchartsHandlingPath.scss'
import Chart from 're-echarts'
import HandlingPathTable from 'components/Table'
...
export class EchartsHandlingPath extends React.Component {
  props: Props;
  ...
  render () {
    ...
    return (
      <div className={classes['EchartsHandlingPath-container']}>
        <Chart option={option} style={{height: '500px'}} />
        {this.props.data.handlingPath &&
          <HandlingPathTable field={field} data={this.props.data.handlingPath.result} />
        }
      </div>
    )
  }
